...
Smart Risk Management: Automated position sizing and capital protection protocols to ensure survival during high market volatility.

Dynamic Options Builder: Advanced tools to deploy and hedge complex options strategies instantly.

<<<<<<< HEAD
Production Safety & Hybrid Control: Gives the trader the flexibility of an algo's speed with the oversight of a manual safety switch (Hybrid Model).
=======
## Api Documentation
...
### Install Package
##### - Clone this repo and install the local package from its root directory.
##### Requirements
...
##### Optional ML / RL dependencies
```bash
pip install xgboost tensorflow stable-baselines3
```
...
```
>>>>>>> 82ba586 (Saving work from codespace)
